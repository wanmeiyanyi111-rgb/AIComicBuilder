...
const SCRIPT_SPLIT_IDEA_REQUIREMENTS = `5. "idea"字段将作为独立AI剧本生成器的唯一输入。它必须极其详细：
   ...
   - 详细但必须与本集实际体量匹配。通常 400-1200 中文字或 250-700 英文词即可，禁止为了凑长度把后续剧情硬并进同一集。`;
...
[
  {
    ...
    "idea": "1) 列出本集所有角色及其定位。2) 逐字复制原文中的关键段落和对白——保留原文措辞，不要概括。3) 添加场景过渡注释和情感节拍标记。保持足够详细，但只覆盖本集，不要把后续大段剧情吞进来。下游剧本生成器无法访问原文——此字段是它的唯一参考。",
