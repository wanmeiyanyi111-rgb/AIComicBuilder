...
export function themeStyleMappingBlock(): string {
  return `【显式风格锁定（优先级高于主题映射）】
- 若输入中出现 "电影级写实" / "写实电影" / "写实电影摄影" / "真人" / "实拍" / "photorealistic" / "cinematic realism" / "live-action" / "风格ID：cinematic_realism"，则必须锁定为电影级写实摄影语言（真实光影与材质），禁止输出 2.5D、动漫赛璐珞、国漫 3D 风格化渲染。
- 若项目风格ID已给出（例如 cinematic_realism），该风格ID优先于主题默认映射和历史漂移标签。
- 若输入显式指定了动漫/漫画/国漫/水墨/像素/油画等风格，也必须锁定该风格，不得被主题反推覆盖。

**主题 → 画风自动映射表**（仅在未显式声明风格时启用；全流水线共用）：
- 仙侠/修真/玄幻 → 东方奇幻电影美术（可写实或国风化，按上下文细化），强调服饰纹理与体积光
- 古风/历史 → 中国风工笔画 / 水墨 / 古典绘画，讲究线条与留白
- 赛博朋克/未来/科幻 → 未来科幻电影美术 / 概念设计，硬表面与发光材质
...
画风判定原则：
1. 先检查显式风格词与项目风格ID（若有），显式风格绝对优先
2. 仅当未声明风格时，才按主题关键词匹配上表
3. 禁止用主题映射覆盖显式风格`;
}
